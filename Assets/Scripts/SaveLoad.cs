...
public static class SaveLoad
{
    ...
    public static void SaveNet(string fileName)
    {
        ...
        {
            List<NeuralNetwork> nets = GameManager.instance.neuralNets;
    ...
    public static void LoadNet(string fileName)
    {
        if (File.Exists(Path.Combine(Application.persistentDataPath, fileName)))
        {
            ...
            GameManager.instance.savedNets = savedNetworks[savedNetworks.Count - 1];
            file.Close();
            GameManager.instance.isLoadingNet = true;
