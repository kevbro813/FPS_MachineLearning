...
public class NeuralNet
{
    ...
    public double[] FeedForward(double[] inputs)
    {
        ...
        for (int lay = 1; lay < neuralNetStructure.Length; lay++) // Iterate through each layer starting with the first hidden layer
        {
            for (int neu = 0; neu < neuronsActivated[lay].Length; neu++) // Iterate through each neuron in the layer
            {
                ...
                for (int wt = 0; wt < neuronsActivated[lay - 1].Length; wt++)
                {
                ...
                neuronsActivated[lay][neu] = ActivationFunctions(neuronSums[lay][neu], lay);
        ...
        return neuronsActivated[neuronsActivated.Length - 1]; // Return the output layer of the neuronsActivated matrix
    }
    public double ActivationFunctions(double value, int layer)
    {
        switch (activationPerLayer[layer])
        {
            ...
            case Settings.LayerActivations.Softmax: // For softmax, do not use an activation function yet since it must use all pre-activation outputs to calculate properly
                return value;
            default:
                return GameManager.instance.math.Relu(value);
        }
    ...
    public void SumGradients(double[][][] weightsGradSums, double[][][] weightsGradSingle, double[][] biasGradSums, double[][] biasGradsSingle)
    {
        for (int lay = 0; lay < weightsGradSums.Length; lay++)
        {
            for (int neu = 0; neu < weightsGradSums[lay].Length; neu++)
            {
                for (int wt = 0; wt < weightsGradSums[lay][neu].Length; wt++)
                {
                    weightsGradSums[lay][neu][wt] += weightsGradSingle[lay][neu][wt];
                }
            }
        }

        for (int lay = 0; lay < biasGradSums.Length; lay++)
        {
            for (int neu = 0; neu < biasGradSums[lay].Length; neu++)
            {
                biasGradSums[lay][neu] += biasGradsSingle[lay][neu];
            }
        }
    }
    /// <summary>
    ...
    public bool Train(DQN dqn, Environment env, Agent agent)
    {
        ...
        for (int i = 0; i < miniBatchSize; i++) // Iterate through each mini batch
        {
            ...
            if (targets != null) // Do not begin training until targets are set
            {
                ...
                SumGradients(weightsGradientSums, weightsGradientSingle, biasGradientSums, biasGradientSingle);
            }
        }
        ...
        // TODO: return weights matrix rather than a bool
        return false;
    }
    ...
    private double[] CalculateTargets(double[] nextStates, double reward, bool done, DQN dqn)
    {
        ...
        double[] qValues = new double[mbs];
        double[] targets = new double[mbs]; // Targets

        qValues = dqn.targetNet.FeedForward(nextStates);

        targets = new double[aq];
    ...
    private void CalculateGradients(double[][] actions, double[] errors, int i, DQN dqn)
    {
        ...
        {
            // Calculate the signal (layer delta) for the output layer
        ...
        //Debug.Log(weightsGradientSingle[2][3][4]);
        //Debug.Log(signals[1][3]);
        //Debug.Log(neuronSums[0][4]);
    }
        ...
        if (!dqn.isConverged)
        {
            ...
            for (int lay = weightsMatrix.Length - 1; lay >= 0; lay--)
            {
                for (int neu = 0; neu < weightsMatrix[lay].Length; neu++)
                {
                    for (int pn = 0; pn < weightsMatrix[lay][neu].Length; pn++)
                    {
                        ...
                        {
                            gradient = gradient * GameManager.instance.settings.gradientThreshold / Math.Abs(gradient); // Clip gradient to prevent exploding and vanishing gradients
                        }
                        ...
                        double deltaWeight = dqn.currentLearningRate * firstMoment[lay][neu][pn] / (Math.Sqrt(secondMoment[lay][neu][pn]) + GameManager.instance.settings.epsilonHat);
            ...
            for (int lay = biasGradientSums.Length - 1; lay > 0; lay--)
            {
                for (int bias = 0; bias < biasGradientSums[lay].Length; bias++)
                {
                    ...
                    {
                       biasGradient = biasGradient * GameManager.instance.settings.gradientThreshold / Math.Abs(biasGradient); // Clip gradient to prevent exploding and vanishing gradients
                    }
                    ...
                    double biasDelta = dqn.currentLearningRate * firstMomentBias[lay][bias] / (Math.Sqrt(secondMomentBias[lay][bias]) + GameManager.instance.settings.epsilonHat);
