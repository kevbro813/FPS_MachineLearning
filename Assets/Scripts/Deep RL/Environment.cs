...
public class Environment
{
    ...
    public double[] GetNextFrame()
    {
        ...
        double[] nextFrame = new double[dqn.frameSize]; // Create a float array  to hold the next frame

        // Populate the next_Frame array with the respective values
        nextFrame[0] = distancesToObstacles[0];
        nextFrame[1] = distancesToObstacles[1];
        nextFrame[2] = distancesToObstacles[2];
        nextFrame[3] = distancesToObstacles[3];
        nextFrame[4] = distancesToObstacles[4];
        nextFrame[5] = distancesToObstacles[5];
        nextFrame[6] = distancesToObstacles[6];
        nextFrame[7] = distancesToObstacles[7];
        nextFrame[8] = tf.position.x;
        nextFrame[9] = tf.position.z;
        //next_Frame[10] = tf.transform.eulerAngles.y;
        //next_Frame[2] = tf.position.y;
    ...
    public int UpdateFrameBuffer(double[] nf) // Pass in the next frame
    {
    ...
    {
        double[][] state = new double[GameManager.instance.settings.framesPerState][]; // Create a new array to hold the state

        double[] flatState = new double[GameManager.instance.settings.framesPerState * dqn.frameSize]; // Flatten state into a 1-D array to input into neural net

        if (frameIndex < 0)
        {
            frameIndex = 0;
        }
        if (fbCount - GameManager.instance.settings.framesPerState >= 0)
        {
            for (int i = 0; i < GameManager.instance.settings.framesPerState; i++) // Loop through the frames, stopping before the last frame
            {  
                state[GameManager.instance.settings.framesPerState - i - 1] = frameBuffer[frameIndex];
            }
            int indx = 0;
            for (int j = 0; j < GameManager.instance.settings.framesPerState; j++)
            {
